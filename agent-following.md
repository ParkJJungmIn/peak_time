...
### 12. PWA & 모바일 UI 지침
- **반응형 기본값**: 모든 화면은 모바일 우선(min-width 360px 가정)으로 설계하고, `min-h-dvh`와 안전영역 패딩을 활용해 PWA에서도 자연스럽게 보이도록 한다.
- **모바일 헤더 시스템**: `src/components/mobile/mobile-header.tsx`와 `mobile-screen.tsx`를 사용해 각 화면 헤더를 선언형으로 구성한다. `MobileScreen`의 `header` prop(타이틀, 서브타이틀, 백버튼, 사용자 정의 슬롯)을 통해 메인/상세 등 다양한 헤더 구성을 재사용한다.
- **히어로 헤더**: 메인 루틴 화면에서 사용하는 칩형 상단 헤더는 `src/components/mobile/hero-header.tsx`를 통해 재사용한다. 좌·우 버튼 라벨/링크를 props로 넘겨 다른 화면에서도 동일한 스타일을 유지한다.
- **히어로 레이아웃**: 홈/보관함 등의 PWA 핵심 화면은 `src/components/layouts/hero-shell.tsx`으로 감싸 동일한 배경, 헤더, 풋터, 스크롤 구조를 유지한다. `tagline`과 본문/푸터 링크만 교체한다.
- **공용 레이아웃**: `MobileScreen`을 기본 컨테이너로 사용해 배경, 여백, 스크롤 로직을 일관화한다. 추가적인 화면별 카드/섹션을 내부에서 구성하고, 필요한 경우 `footer` prop으로 하단 CTA를 구성한다.
- **PWA 고려사항**: 헤더 버튼 크기는 44px 이상, 터치 영역을 확보하고, 브라우저/앱 타이틀바 겹침을 피하기 위해 상단 padding을 조정한다. 오프라인 메시지, 네트워크 재시도 UI를 고려해 API 호출 결과를 사용자에게 명확히 표시한다.
